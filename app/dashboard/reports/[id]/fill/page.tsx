...
export default function FillReportPage({
    ...
    async function fetchTemplate() {
      try {
        ...
        if (data) {
          ...
          data.fields.forEach((f) => (initialData[f.key] = ""))
  ...
  const processGeneration = async (mode: "docx" | "pdf") => {
    ...
    try {
      ...
      const baseName = `${template.name}_${Date.now()}`
      ...
      if (mode === "docx") {
        const { fileUrl } = await templateService.uploadGeneratedFile(
          docxBlob,
          wordName,
          user.uid
        )
      ...
      <Card className="bg-card/60 overflow-hidden border-none shadow-2xl backdrop-blur-2xl">
        ...
        <CardFooter className="bg-muted/20 flex flex-col gap-8 p-12 pt-6 sm:flex-row">
          <Button
            ...
            size="lg"
            className="shadow-brand-500/40 h-20 flex-1 gap-4 rounded-[2rem] text-xl font-black shadow-2xl"
          >
            {isGenerating ? (
          ...
          <Button
            variant="outline"
            size="lg"
            className="border-brand-500/20 text-brand-400 hover:bg-brand-500 h-20 flex-1 gap-4 rounded-[2rem] border-2 text-xl font-black shadow-xl transition-all hover:text-white"
